...
TMP_DIR=$(mktemp -d -t php-build-plugin-phpcsfixer)
...
echo "Installing php-build-plugin-phpcsfixer in \"${PREFIX}\""
...
((wget https://github.com/rogeriopradoj/php-build-plugin-phpcsfixer/archive/master.tar.gz && tar -vzxf master.tar.gz) 2>/dev/null) || ((curl -LO https://github.com/rogeriopradoj/php-build-plugin-phpcsfixer/archive/master.tar.gz && tar -vzxf master.tar.gz) 2>/dev/null) || ((git clone https://github.com/rogeriopradoj/php-build-plugin-phpcsfixer.git php-build-plugin-phpcsfixer-master) 2>/dev/null) || (echo ""; echo "***ERROR***: You do not have either wget, neither curl or git installed on your system to download the needed files."; echo "Please, check that, and after, try to install this plugin again."; exit;)
echo " Done."

echo -n "  - Making the plugin executable..."
cd ${TMP_DIR}/php-build-plugin-phpcsfixer-master
...
echo "------------"
echo ""
echo ' The plugin phpcsfixer has been installed. Your next builds made with php-build will have command php-cs-fixer enabled.'